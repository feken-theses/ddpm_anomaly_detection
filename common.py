...
def create_images_grid(images, rows, cols):
    ...
    w, h = images[0].size
    grid_rgb = Image.new('RGB', size=(cols * w, rows * h))
    grid=grid_rgb.convert('L')
    for i, image in enumerate(images):
        grid.paste(image, box=(i % cols * w, i // cols * h))
    return grid


def create_sampling_animation(model, pipeline, config, interval=5, every_nth_image=1, rows=1, cols=14):
